...
class _CartScreenState extends ConsumerState<CartScreen> {
  ...
  Future<void> _startRazorpayPayment(BuildContext context) async {
    ...
    try {
      ...
      final localEnvKey = PaymentConfig.razorpayKeyId.trim();
      final backendKey = order['key']?.toString().trim() ?? '';
      final checkoutKey = localEnvKey.isNotEmpty ? localEnvKey : backendKey;
      if (checkoutKey.isEmpty) {
        throw StateError(
          'Payment gateway key is not configured in user app env or backend env.',
        );
      }
