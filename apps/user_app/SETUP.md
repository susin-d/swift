...
# Option 1: Use default backend URL (localhost:3000) from .env
flutter run

# Option 2: Point to a different backend URL
# Edit user_app/.env for development or user_app/.env.production for release
...
The `.env` file documents your local configuration and is bundled for runtime loading:
```
BACKEND_API_URL=http://localhost:3000/api/v1
```

Production values should go in `.env.production`.
For the current deployment, that value is `https://swift-campus.vercel.app/api/v1`.
...
- Verify backend URL: check `user_app/.env` or `user_app/.env.production`
