...
- Bottom navigation now follows the screenshot layout: Home, Browse, Orders, Wallet, and Account
- Browse uses the search screen as a discovery grid when no query is entered
- Wallet has a dedicated balance/transactions screen backed by the user session and order history
...
Payment gateway (Razorpay) is now fully backend-driven. The app does not require a direct Razorpay build flag.

The app reads runtime configuration from:
