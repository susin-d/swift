...
describe('Admin Controller - Vendor Moderation', () => {
    ...
    it('getAdminOrders falls back to reduced shape when joins are unavailable', async () => {
        const countSelectStub = Sinon.stub().resolves({ count: 1, error: null });

        const primaryRangeStub = Sinon.stub().resolves({
            data: null,
            error: { message: "Could not find a relationship between 'orders' and 'users' in the schema cache" },
        });
        const primaryOrderStub = Sinon.stub().returns({ range: primaryRangeStub });
        const primarySelectStub = Sinon.stub().returns({ order: primaryOrderStub });

        const fallbackOrders = [{ id: 'o-10', status: 'pending' }];
        const fallbackRangeStub = Sinon.stub().resolves({ data: fallbackOrders, error: null });
        const fallbackOrderStub = Sinon.stub().returns({ range: fallbackRangeStub });
        const fallbackSelectStub = Sinon.stub().returns({ order: fallbackOrderStub });

        fromStub.onCall(0).returns({ select: countSelectStub } as any);
        fromStub.onCall(1).returns({ select: primarySelectStub } as any);
        fromStub.onCall(2).returns({ select: fallbackSelectStub } as any);
        ...
        Sinon.assert.calledWithExactly(primarySelectStub, '*, users(name, email), vendors(name), order_items(quantity, unit_price)');
        Sinon.assert.calledWithExactly(fallbackSelectStub, '*');
        expect(reply.send.firstCall.args[0]).toEqual({
    ...
    it('getAdminAuditLogs returns empty page when admin_logs table is unavailable', async () => {
        const countSelectStub = Sinon.stub().resolves({
            count: null,
            error: { message: 'relation "admin_logs" does not exist' },
        });
