...
export const CONTRACT_ENDPOINTS: ContractEndpoint[] = [
    ...
    {
        id: 'admin.support.tickets.timeline',
        method: 'GET',
        path: '/api/v1/admin/support/tickets/:id/timeline',
        owner: 'admin_app',
        auth: 'admin',
        response: {
            description: 'Timeline entries for an admin support ticket.',
            fields: [
                { name: 'ticketId', type: 'string(uuid)', required: true, description: 'Support ticket id.' },
                { name: 'events', type: 'array<object>', required: true, description: 'Timeline event entries.' }
            ]
        }
    },
    {
        ...
        auth: 'admin',
        request: {
            description: 'Broadcast a notification to users, vendors, or both audiences.',
            fields: [
                { name: 'title', type: 'string', required: true, description: 'Notification title.' },
                { name: 'body', type: 'string', required: true, description: 'Notification body.' },
                { name: 'audience', type: 'string', required: false, description: 'Target audience: user|vendor|both.' },
                { name: 'type', type: 'string', required: false, description: 'Notification type tag.' },
                { name: 'metadata', type: 'object', required: false, description: 'Optional metadata payload.' }
            ]
        },
        response: {
            description: 'Broadcast delivery summary.',
            fields: [
                { name: 'message', type: 'string', required: true, description: 'Broadcast status message.' },
                { name: 'audiences', type: 'array<string>', required: true, description: 'Resolved audience list.' },
                { name: 'recipients', type: 'number', required: true, description: 'Total recipient count.' },
                { name: 'sent', type: 'number', required: true, description: 'Successfully delivered notification count.' },
                { name: 'failed', type: 'number', required: true, description: 'Failed delivery count.' }
            ]
        }
    },
    {
