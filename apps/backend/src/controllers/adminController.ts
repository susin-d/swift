...
const getErrorText = (error: unknown): string => String((error as any)?.message || error || '').toLowerCase();
...
const isSchemaDriftError = (error: unknown): boolean => {
    ...
    return text.includes('relation') ||
        ...
        text.includes('not found in the schema');
};

const isMissingAdminLogsStorage = (error: unknown): boolean => {
    const text = getErrorText(error);
    return text.includes('admin_logs') && (text.includes('relation') || text.includes('does not exist'));
};
...
export const getPendingVendors = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (error) {
        ...
        // Fallback for deployments where optional owner joins are unavailable.
        ...
        // If schema drift impacts status filtering itself, degrade safely.
        if (isSchemaDriftError(fallback.error)) {
            return reply.send({
                vendors: [],
                page,
                limit,
...
export const getAdminOrders = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const primary = status ? await baseQuery.eq('status', status) : await baseQuery;
    let data = primary.data;
    let error = primary.error;

    if (error && isSchemaDriftError(error)) {
        // Fallback for deployments where optional joins are unavailable.
        const fallbackBase = supabase
            .from('orders')
            .select('*')
            .order('created_at', { ascending: false })
            .range(from, to);
        const fallback = status ? await fallbackBase.eq('status', status) : await fallbackBase;
        data = fallback.data;
...
export const getAdminAuditLogs = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const { count, error: countError } = await countQuery;
    if (countError) {
        if (!isMissingAdminLogsStorage(countError)) {
            throw countError;
        }
    ...
    if (error) {
        if (!isMissingAdminLogsStorage(error)) {
            throw error;
        }
