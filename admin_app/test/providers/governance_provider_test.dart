...
void main() {
  group('AuditGovernance — action logging and traceability', () {
    test(
      'records moderation action with reason, outcome, and admin identity',
      () {
        final auditLog = {
          'action_id': 'action-1',
          'admin_id': 'admin-001',
    ...
    test('enables searching audit log by date, admin, or target', () {
      ...
      final adminOneActions = entries
          .where((e) => e['admin_id'] == 'admin-001')
          .toList();
  ...
  group('AuditGovernance — decision traceability and impact assessment', () {
    test(
      'captures before/after state for consequential moderation decisions',
      () {
        ...
        final before = decision['before'] as Map<String, Object>;
        final after = decision['after'] as Map<String, Object>;
        expect(before['status'], equals('active'));
        expect(after['status'], equals('suspended'));
      },
    );

    test(
      'quantifies business impact of governance actions (orders delayed, revenue lost)',
      () {
        final impact = {
  ...
  group('AuditGovernance — workflow and escalation', () {
    test(
      'supports escalation workflow for sensitive actions (requires approval)',
      () {
        final escalation = {
          'escalation_id': 'esc-1',
  ...
  group('AuditGovernance — compliance and reporting', () {
    test('generates audit report with summary of actions and risk signals', () {
      final report = {
        ...
        'risk_flags': [
          'vendor_chargebacks_spike',
          'delayed_settlements_2_vendors',
        ],
